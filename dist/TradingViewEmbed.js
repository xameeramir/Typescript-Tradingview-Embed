...
function widgetAdvancedChartOnload(script, count, widgetConfig) {
    ...
    var symbol = config.symbol, timezone = config.timezone, interval = config.interval, style = config.style, toolbar_bg = config.toolbar_bg, enable_publishing = config.enable_publishing, withdateranges = config.withdateranges, range = config.range, hide_side_toolbar = config.hide_side_toolbar, allow_symbol_change = config.allow_symbol_change, details = config.details, hotlist = config.hotlist, calendar = config.calendar, show_popup_button = config.show_popup_button, popup_width = config.popup_width, popup_height = config.popup_height, no_referral_id = config.no_referral_id, referral_id = config.referral_id, width = config.width, height = config.height, colorTheme = config.colorTheme, locale = config.locale, autosize = config.autosize, news = config.news, studies = config.studies, watchlist = config.watchlist;
...
function widgetCompanyProfile(widgetConfig, copyrightLink) {
    ...
    var symbol = config.symbol, width = config.width, height = config.height, colorTheme = config.colorTheme, isTransparent = config.isTransparent, locale = config.locale, autosize = config.autosize;
    if (autosize) {
        width = "100%";
        height = "100%";
    }
...
function widgetEconomicCalendar(widgetConfig, copyrightLink) {
    ...
    var width = config.width, height = config.height, colorTheme = config.colorTheme, isTransparent = config.isTransparent, locale = config.locale, autosize = config.autosize, importanceFilter = config.importanceFilter;
    if (autosize) {
        width = "100%";
        height = "100%";
    }
...
function widgetForexCrossRates(widgetConfig, copyrightLink) {
    ...
    var currencies = config.currencies, width = config.width, height = config.height, locale = config.locale, autosize = config.autosize;
    if (autosize) {
        width = "100%";
        height = "100%";
    }
...
function widgetFundamentalData(widgetConfig, copyrightLink) {
    ...
    var symbol = config.symbol, largeChartUrl = config.largeChartUrl, width = config.width, height = config.height, colorTheme = config.colorTheme, isTransparent = config.isTransparent, displayMode = config.displayMode, locale = config.locale, autosize = config.autosize;
    if (autosize) {
        width = "100%";
        height = "100%";
    }
...
function widgetMarketData(widgetConfig, copyrightLink) {
    ...
    var symbolsGroups = config.symbolsGroups, largeChartUrl = config.largeChartUrl, width = config.width, height = config.height, locale = config.locale, autosize = config.autosize;
    if (autosize) {
        width = "100%";
        height = "100%";
    }
...
function widgetMarketOverview(widgetConfig, copyrightLink) {
    ...
    var tabs = config.tabs, dateRange = config.dateRange, plotLineColorGrowing = config.plotLineColorGrowing, plotLineColorFalling = config.plotLineColorFalling, gridLineColor = config.gridLineColor, scaleFontColor = config.scaleFontColor, belowLineFillColorGrowing = config.belowLineFillColorGrowing, belowLineFillColorFalling = config.belowLineFillColorFalling, symbolActiveColor = config.symbolActiveColor, showChart = config.showChart, largeChartUrl = config.largeChartUrl, width = config.width, height = config.height, colorTheme = config.colorTheme, isTransparent = config.isTransparent, locale = config.locale, autosize = config.autosize;
    if (autosize) {
        width = "100%";
        height = "100%";
    }
...
function widgetMiniChart(widgetConfig, copyrightLink) {
    ...
    var symbol = config.symbol, dateRange = config.dateRange, trendLineColor = config.trendLineColor, underLineColor = config.underLineColor, largeChartUrl = config.largeChartUrl, width = config.width, height = config.height, colorTheme = config.colorTheme, isTransparent = config.isTransparent, locale = config.locale, autosize = config.autosize;
    if (autosize) {
        width = "100%";
        height = "100%";
    }
...
function widgetScreener(widgetConfig, copyrightLink) {
    ...
    var defaultColumn = config.defaultColumn, defaultScreen = config.defaultScreen, market = config.market, showToolbar = config.showToolbar, largeChartUrl = config.largeChartUrl, width = config.width, height = config.height, colorTheme = config.colorTheme, isTransparent = config.isTransparent, locale = config.locale, autosize = config.autosize;
    if (autosize) {
        width = "100%";
        height = "100%";
    }
...
function widgetScreenerCryptocurrrency(widgetConfig, copyrightLink) {
    ...
    var defaultColumn = config.defaultColumn, screener_type = config.screener_type, displayCurrency = config.displayCurrency, width = config.width, height = config.height, colorTheme = config.colorTheme, isTransparent = config.isTransparent, locale = config.locale, autosize = config.autosize;
    if (autosize) {
        width = "100%";
        height = "100%";
    }
...
function widgetSymbolOverviewOnload(script, count, widgetConfig) {
    ...
    var symbols = config.symbols, greyText = config.greyText, gridLineColor = config.gridLineColor, fontColor = config.fontColor, underLineColor = config.underLineColor, trendLineColor = config.trendLineColor, width = config.width, height = config.height, locale = config.locale, chartOnly = config.chartOnly, autosize = config.autosize;
    if (autosize) {
        width = "100%";
        height = "100%";
    }
...
function widgetStockMarket(widgetConfig, copyrightLink) {
    ...
    var dateRange = config.dateRange, exchange = config.exchange, plotLineColorGrowing = config.plotLineColorGrowing, plotLineColorFalling = config.plotLineColorFalling, gridLineColor = config.gridLineColor, scaleFontColor = config.scaleFontColor, belowLineFillColorGrowing = config.belowLineFillColorGrowing, belowLineFillColorFalling = config.belowLineFillColorFalling, symbolActiveColor = config.symbolActiveColor, showChart = config.showChart, largeChartUrl = config.largeChartUrl, width = config.width, height = config.height, colorTheme = config.colorTheme, isTransparent = config.isTransparent, locale = config.locale, autosize = config.autosize;
    if (autosize) {
        width = "100%";
        height = "100%";
    }
...
function widgetTechnicalAnalysis(widgetConfig, copyrightLink) {
    ...
    var symbol = config.symbol, showIntervalTabs = config.showIntervalTabs, interval = config.interval, largeChartUrl = config.largeChartUrl, width = config.width, height = config.height, colorTheme = config.colorTheme, isTransparent = config.isTransparent, locale = config.locale, autosize = config.autosize;
    if (autosize) {
        width = "100%";
        height = "100%";
    }
